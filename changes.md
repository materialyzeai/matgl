...
- **`matgl.utils.training.MatGLTrainer` (new, PyG-only).** First dataset-level Hugging Face integration in matgl: a configure-once / fit-when-asked wrapper that lives next to `PotentialLightningModule` and `fit_element_refs`. `__init__` stores hyperparameters; nothing heavy runs until `fit(dataset=...)`.
  - **MatPES loaders** (HF: `materialyze/matpes`): `MatGLTrainer.load_matpes_dataset(version="r2SCAN-2025.2", split=None)`, `load_matpes_splits(version)`, `load_matpes_element_refs(version, element_types=...)`. Each is a static method usable without instantiating a trainer; element references are reorderable to the caller's `element_types`.
  - **extxyz loaders** (any HF dataset or local file): `MatGLTrainer.load_extxyz_dataset(...)` and `load_extxyz_splits(...)` accept either `path=...` for a local file or `(repo_id, filename)` for a Hub download. `.extxyz`, `.xyz`, and `.tar.gz` / `.tgz` / `.tar` (with one or more extxyz members) are handled transparently. Tarballs whose member filenames carry `_train` / `_test` / `_valid` (or `_val`) suffixes split into the canonical trio via `load_extxyz_splits`. Stresses are auto-included only when every frame exposes one, so cluster / dimer datasets like `materialyze/mlip-lr-benchmarks` `cp_dimer.tar.gz` yield a forces-only dataset and `MGLDataLoader`'s auto-detect picks `include_stress=False`.
  - **Trainer**: `MatGLTrainer(model, accelerator="auto", max_epochs=100, ...)` accepts the full Lightning placement vocabulary (`"auto"` / `"cpu"` / `"gpu"` / `"cuda"` / `"mps"` / `"tpu"`). `trainer.fit(dataset, *, format="auto", atomrefs=None, save_path=None, push_to_hub=None)` is the single entry point:
    - `dataset` accepts a HF spec (`(repo_id, filename)` tuple or a mapping with `"repo_id"` / `"filename"` / `"revision"` / `"token"` / `"cache_dir"`), a local file path (str / `Path`), a pre-built `MGLDataset`, or a `{"train", "valid", "test"}` mapping of pre-built canonical splits.
    - `format` is one of `"auto"` (the default; infers from filename suffix — `.json` / `.json.gz` -> matpes, `.extxyz` / `.xyz` / `.tar.gz` / `.tgz` / `.tar` -> extxyz), `"matpes"`, or `"extxyz"`.
    - `atomrefs` accepts an `np.ndarray`, an in-memory `{"element_types": [...], "refs": [...]}` dict, an `AtomRef` instance, an HF spec, or a local path; the trainer reorders the result to `model.element_types` automatically. `None` (default) skips offsets.
    - When the resolved dataset has no stress labels (e.g. `cp_dimer.tar.gz`), the stress loss term is auto-disabled for that fit with a one-line warning so cluster / dimer datasets work with the default `stress_weight=0.1` out of the box.
