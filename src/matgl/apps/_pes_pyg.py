...
class Potential(nn.Module, IOMixIn):
    ...
    def forward(
        ...
        if self.calc_stresses and grads is not None:
            volume = torch.abs(torch.det(lattice.float())).to(lattice.dtype)
            sts = grads[1]
            ...
            stresses = torch.stack(sts) if isinstance(sts, list) else sts  # type:ignore[call-overload]

        if self.debug_mode and grads is not None:
            return total_energies, grads[0], grads[1]

        if self.calc_magmom:
            return total_energies, forces, stresses, hessian, getattr(g, "magmom", None)
