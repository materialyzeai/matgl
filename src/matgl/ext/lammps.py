...
class _SmoothSBFExpansion(nn.Module):
    ...
    helpers (sympy-lambdified ``funcs`` list), which blocks ``torch.jit.script``.
...
class _TensorNetKernel(nn.Module):
    ...
    def __init__(self, model: nn.Module) -> None:
        ...
        # the kernel survives torch.jit.script. Gaussian / ExpNormal /
...
class _M3GNetKernel(nn.Module):
    ...
    def __init__(self, model: nn.Module) -> None:
        ...
        # callables that don't survive ``torch.jit.script``. We
        ...
        # bond_expansion's RBF max_n, kept so forward can rebuild the smooth-SBF
        # expansion in pure tensor ops (M3GNet export only supports the
        # SphericalBessel rbf_type; see the validation below).
...
class LAMMPSMatGLModel(nn.Module):
    ...
    def __init__(
        ...
        """Wrap a MatGL ``Potential`` for LAMMPS export.

        Args:
            potential: A trained MatGL ``Potential`` whose inner model is a PyG
                ``TensorNet`` (``use_warp=False``) or ``M3GNet`` with an
                extensive head. ZBL repulsion, magmom, and charge heads are not
                supported.
            dtype: Buffer dtype for the wrapper; use ``torch.float32`` or
                ``torch.float64``.

        Raises:
            ValueError: If the inner model is Warp-backed TensorNet or has an
                intensive head.
            NotImplementedError: If the inner model architecture or an enabled
                ``Potential`` head (ZBL/magmom/charge) is not exportable.
        """
        super().__init__()
...
def export_lammps_model(
    ...
    Args:
        ...
        dtype: Wrapper buffer dtype; use ``torch.float32`` or ``torch.float64``.
