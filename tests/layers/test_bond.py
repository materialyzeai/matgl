...
def test_gaussian(graph_MoS, graph_CO):
    ...
    assert bond_basis.shape == (g1.bond_dist.shape[0], 10)

    bond_basis = bond_expansion(g2.bond_dist)
    assert bond_basis.shape == (g2.bond_dist.shape[0], 10)
...
def test_spherical_bessel_with_smooth(graph_MoS, graph_CO):
    ...
    bond_basis = bond_expansion(g1.bond_dist)
    assert bond_basis.shape == (g1.bond_dist.shape[0], 3)

    bond_basis = bond_expansion(g2.bond_dist)
    assert bond_basis.shape == (g2.bond_dist.shape[0], 3)
...
def test_spherical_bessel(graph_MoS, graph_CO):
    ...
    bond_basis = bond_expansion(g1.bond_dist)
    assert bond_basis.shape == (g1.bond_dist.shape[0], 9)

    bond_basis = bond_expansion(g2.bond_dist)
    assert bond_basis.shape == (g2.bond_dist.shape[0], 9)
