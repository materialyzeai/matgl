...
def test_grace_forward_returns_scalar(graph_MoS):
    torch.manual_seed(0)
    ...
    model = _make_grace().to(matgl.float_th)
    output = model(graph)
    _check_scalar_output(output)


def test_grace_save_load_roundtrip(graph_MoS, tmp_path):
    torch.manual_seed(0)
    structure, graph, _ = graph_MoS
    lat = torch.tensor(np.array([structure.lattice.matrix]), dtype=matgl.float_th)
    _set_pos_and_pbc(graph, lat)
...
def test_grace_translation_invariance(graph_MoS):
    torch.manual_seed(0)
    structure, graph, _ = graph_MoS
    lat = torch.tensor(np.array([structure.lattice.matrix]), dtype=matgl.float_th)
    _set_pos_and_pbc(graph, lat)
    ...
    graph.pos = pos_orig  # restore for downstream tests
    assert torch.allclose(e0, e1, atol=1e-6)


def test_grace_rotation_invariance(graph_MoS):
    torch.manual_seed(0)
    structure, graph, _ = graph_MoS
    lat = torch.tensor(np.array([structure.lattice.matrix]), dtype=matgl.float_th)
    _set_pos_and_pbc(graph, lat)
...
def test_grace_with_potential_returns_forces_and_stresses(graph_MoS):
    torch.manual_seed(0)
    structure, graph, _ = graph_MoS
    lat = torch.tensor(np.array([structure.lattice.matrix]), dtype=matgl.float_th)
    model = _make_grace().to(matgl.float_th)
    ...
    # Newton's 3rd law: forces should sum to ~0 across atoms.
