"""DGL<->PyG parity test for ``M3GNet``.

Loads a state-dict + golden output from ``tests/parity_data/m3gnet_parity.pt``
...
ARTIFACT = Path(__file__).resolve().parents[1] / "parity_data" / "m3gnet_parity.pt"
...
def parity_artifact():
    ...
    if not ARTIFACT.exists():
        pytest.skip(f"Parity artifact missing: {ARTIFACT}. Generate via tests/parity_data/gen_m3gnet_parity.py.")
...
def test_m3gnet_dgl_pyg_parity(parity_artifact):
    ...
    assert torch.allclose(output, expected, atol=1e-5, rtol=1e-5), (
        f"M3GNet parity broken on backend={matgl.config.BACKEND}: "
